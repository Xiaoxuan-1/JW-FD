...
def process_batch(batch):
    ...
    for img_path in batch:
        try:
            img = imageio.imread(img_path).astype(float)
    ...
    for item in preloaded:
        ...
        try:
            if img.ndim == 3 and img.shape[2] >= 3:
                ...
                gray_image = sum(img[:, :, i] * weights[i] for i in range(3))
            elif img.ndim == 2:
                gray_image = img
            else:
...
def generate_dataset():
    ...
    filter_msg = f"年份过滤 [{START_YEAR}, {END_YEAR}]：{total_before} -> {len(png_files)}"
    ...
    if PARAMS['use_multiprocess']:
        ...
        init_worker(params)
        for png_file in tqdm(png_files, desc="提取PNG特征"):
